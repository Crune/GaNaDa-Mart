...
public class OrderDao {
	...
	public void insertOrder( List<CartDao> lists,
        ...
        try {
            ...
            pstmt = conn.prepareStatement("select max(order_id) from order");
            ...
            for(int i=0; i<lists.size();i++){
            	...
            	CartDao cart = lists.get(i);
            	
            	sql = "insert into order (order_id,order_name,item_id,item_name,item_price,order_count,";
                sql += "book_image,order_date,account,deliveryName,deliveryTel,deliveryAddress)";
                sql += " values (?,?,?,?,?,?,?,?,?,?,?,?)";
                pstmt = conn.prepareStatement(sql);
            
                pstmt.setLong(1, buyId);
                pstmt.setString(2, id);
               pstmt.setInt(3, cart.getitem_id());
                pstmt.setString(4, cart.getitem_name());
                pstmt.setInt(5, cart.getitem_price());
                pstmt.setByte(6, cart.getitem_count());
                pstmt.setString(7, cart.getitem_image());
                ...
                pstmt = conn.prepareStatement(
                		"select item_count from product where _id=?");
                ...
                sql = "update product set item_count=? where product_id=?";
                pstmt = conn.prepareStatement(sql);
           
                pstmt.setShort(1, nowCount);
    			pstmt.setInt(2, cart.getitem_id());
