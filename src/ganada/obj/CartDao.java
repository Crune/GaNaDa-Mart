...
import ganada.core.*;
	...
	try {
    	sql.inSql("ID", "BANNER_SEQ.NEXTVAL");
    		sql.in("CART_ID", cart.getCart_id()); // 장바구니 아이디
    ...
    public Cart getCart(String cart_id) throws Exception {
    	DB db = new DB();
    	Cart cart = null;
        try {
        	db.S("*", "CART", "CODE=?").var(cart_id).exe();
            if (db.next()) {
            cart = new Cart();
    ...
    */
    ...
    public void updateCart(Cart cart) throws Exception {
   	...
   	try {
         ...
         sql.set("ITEM_CL", cart.getItem_num());
         sql.set("ITEM_SIZE", cart.getItem_num());
         sql.set("ITEM_CNT", cart.getItem_cnt());
