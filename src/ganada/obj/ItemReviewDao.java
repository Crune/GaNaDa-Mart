...
public class ItemReviewDao {
	...
	public void insertArticle(ItemReview article){
		...
		try{
			...
			sql = "insert into review(num,itemnum,itemname,writer,subject,lik,bad,siz,comfortable,wid,dura,itemsize,wei,hei,age,mail,content,star,passwd,readcount,ip,reg_date,REF,RE_STEP,RE_LEVEL) values(review_seq.NEXTVAL,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			pstmt = conn.prepareStatement(sql);
